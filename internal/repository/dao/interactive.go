...
func (G *GROMInteractiveDAO) GetByIds(ctx context.Context, biz string, ids []int64) ([]Interactive, error) {
	var inters []Interactive
	err := G.db.WithContext(ctx).Where("biz = ? AND biz_id IN ?", biz, ids).Find(&inters).Error
	return inters, err
}
