...
import (
	...
	"errors"
	"fmt"
	"github.com/Tuanzi-bug/tuan-book/internal/repository/cache/redismocks"
	...
	testCases := []struct {
	...
	}{
		{
			...
			mock: func(ctrl *gomock.Controller) redis.Cmdable {
				...
				cmd.SetErr(nil)
				cmd.SetVal(int64(0))
				res.EXPECT().Eval(gomock.Any(), luaSetCode, []string{keyFunc("test", "15212345678")}, []any{"123456"}).Return(cmd)
				return res
			},
